...
thegui2 := Gui("+AlwaysOnTop +SysMenu +border")
thegui2.backcolor := "0x3b3b3b"
thegui2.SetFont("s20 cwhite w800 Underline", "Tahoma")
letext := thegui2.add("text", "y5", " Settings ")
thegui2.SetFont("s13 cwhite norm w800", "Tahoma")
legroupboxthatcontrolitall := thegui2.add("groupbox", "w140 y47 h" 220-47-5)
makeitallalign(legroupboxthatcontrolitall, 150)
dothisnumberalign := thegui2.add("text","y45","Number Color")
makeitallalign(dothisnumberalign, 150)
dangitsthecolortolerance := thegui2.add("text","y150","Color Tolerance")
makeitallalign(dangitsthecolortolerance, 150)

thegui2.SetFont("s11 cwhite norm", "Tahoma")
thepixelcolor := thegui2.add("edit", "y75 w120 +background3b3b3b", thecolorforthenumberthing)
thetolerancething := thegui2.add("edit", "y180 w120 +background3b3b3b", thetolerancesettingsyeah)
getthepixelcolor := thegui2.add("button", "y105 w100 h25 +background3b3b3b", "Get Color")
getthepixelcolor.onevent("click", (*) => GetTheColorForSomething())
savethegui2settings := thegui2.add("button", "y222 w120 h30 +background3b3b3b", "Save Settings")
savethegui2settings.onevent("click", (*) => UpdateOtherSettings())
makeitallalign(savethegui2settings, 150)
makeitallalign(thepixelcolor, 150)
makeitallalign(getthepixelcolor, 150)
makeitallalign(thetolerancething, 150)
maketransparenttext(thepixelcolor, "pixel color")
maketransparenttext(thetolerancething, "Tolerance")
makeitallalign(letext, 150)
thegui2.title := "Settings"
return
...
global
	...
	fatarrowsettimer := (*) => ttp("Press `"R`" once your done`n`n(put your mouse in the color)`n(you want the numbers color)",,, 1)
	settimer fatarrowsettimer, 150
	keywait "R", "D"
	mousegetpos &thenofishconfirmationx, &thenofishconfirmationy
	thepixelcolor.value := PixelGetColor(thenofishconfirmationx, thenofishconfirmationy)
	keywait "R"
...
global
	...
	ytreshhold := 300 + ystarting
		for i, v in Appraiseable
		{
			try {
				if (instr(v, "(Event)")) {
				vt := trim(strreplace(v, "(Event)", ""))
				thegui.SetFont("s10 c8795ff", "Tahoma")
				maps["cb" . vt] := thegui.Add("CheckBox", "v" vt " x" xpos " y" ypos " h18", vt)
				}else{
...
global
	loop loopamount {
		...
		for k, v in maps
		{
			if (k != "cbBig/Giant") {
				if (v.value == 1) {
					FindMutation(trim(strreplace(k, "cb", "")))
				}
			}else{
				if (v.value == 1) {
					FindBigOrGiant()
...
send "{g}"
sleep 300
if (Pixelsearch(&template, &template, thenofishconfirmationx-40, thenofishconfirmationy-20, thenofishconfirmationx+40, thenofishconfirmationy+20, 0xA8FF95, thetolerancesettingsyeah)) {
...
global
	var1234 := ""
	SplittedHTML := strsplit(GetHTML("https://fischipedia.org/wiki/Mutations"), "</td></tr>")
	for i, v in SplittedHTML
	{
		if (instr(v, "</a></span></td>") and instr(v, "<td class=`"appraisable-true`" title=`"Yes`">")) {
			if RegExMatch(v, ">([^<]+)<\/a><\/span><\/td>", &LeMutation) {
				var1234 .= trim(LeMutation[1]) ","
			}
		}
		if (instr(v, "</a></span></td>") and instr(v, "<td class=`"appraisable-temp`" title=`"Temporarily`">")) {
			if RegExMatch(v, ">([^<]+)<\/a><\/span><\/td>", &LeMutation) {
				var1234 .= trim(LeMutation[1]) " (Event),"
			}
		}
	}
	if (trim(var1234) == "") {
...
global
	...
	thegui.Add("Text", "x" otherstarting+8 " y10 w200", "Auto Appraisal V8")
	...
	extrasettings := thegui.add("button", "x" otherstarting+150 " y139 w25", ">")
	extrasettings.OnEvent("Click", (*) => ShowAnotherUI())
	OpenTheMutationUp := thegui.add("button", "x" otherstarting+150 " y207 w25", ">")
	OpenTheMutationUp.OnEvent("Click", (*) => run(file2))
	...
	thegui.Title := "Auto Appraisal v8"
...
global
	WinGetPos( &Xx2, &Yy2, &Width, &Height, thegui)
	thegui2.show("x" Xx2+Width " y" Yy2+20 " w150 h260")
	dothisnumberalign.focus()
}
...
makeitallalign(GUINAME, thewidthofgui) {
global
	GUINAME.getpos( ,, &tempyheay)
	GUINAME.move(floor(thewidthofgui/2-tempyheay/2))
}
...
maketransparenttext(thename, message) {
global
	SendMessage(0x1501, false, StrPtr(message), thename.Hwnd)
}