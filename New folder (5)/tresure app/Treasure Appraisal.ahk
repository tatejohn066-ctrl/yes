...
h := 0, m := 0, s := 0
MoneyUsage := 0
File1 := "TreasureAppraisalSettingsStuffDontTouch.txt"
...
TakePRESSETLOCKATION() {
global
thecooking := ""
thepresetloc := []
try winactivate("Roblox")
	loop 7 {
	FunctionICopyFromAutoAppraisal := (*) => ttp("Pick row " A_Index "`n`nPress `"" "r" "`" to continue",,,20)
	settimer FunctionICopyFromAutoAppraisal, 100
	keywait "r", "D"
	mousegetpos(&qweedasasdw,&oijasdsojw)
	thecooking .= qweedasasdw "," oijasdsojw "-"
	ttp A_Index, qweedasasdw, oijasdsojw, A_Index
	keywait "r"
	settimer FunctionICopyFromAutoAppraisal, 0
	ttp ,,, 20
	}
	asdfw123 := strsplit(thecooking, "-")
	for i, v in asdfw123
	{
		if (trim(v) != "") {
		thepresetloc.push(trim(v))
		}
	}
	msgbox "Preset done", "Success", "0x40030"
	cleartooltip
	Updatesettings
}
...
global
	...
	msgbox "buh config preset location first before you yap in question channel", "error", "0x40030"
	return
	}
	...
	previoustext := ""
	MoneyUsage := 0
	highlight(ocrdetectionthingx, ocrdetectionthingy, ocrdetectionthingw, ocrdetectionthingh,,"0xFCEF71")
	loop {
		yloop := 1
		MoneyUsage := (A_Index-1)*1000
		if !(goalreach) {
			loop {
				if (yloop >= 8) {
					break
				}
				if (pixelsearch(&xposss, &yposss, reappraisex, reappraisey, reappraisex2, reappraisey2, bluecolor, 3)) {
					if (MoneyUsage < moneyThreshold) {
					loop 30 {
						click xposss, yposss
						sleep 100
						if !(pixelsearch(&xposss, &yposss, reappraisex, reappraisey, reappraisex2, reappraisey2, bluecolor, 3)) {
				...
				click thepresetloc[yloop]
				yloop++
				sleep delay
				TheMultiplier := RegExReplace(RegExReplace(RegExReplace(OCR.FromRect(ocrdetectionthingx, ocrdetectionthingy, ocrdetectionthingw, ocrdetectionthingh, "en-us", 2).Text, "i)[OQ]", "0"), "i)[Il|]", "1"), "[^0-9.]", "")
				tries := 0
				loop {
					if (TheMultiplier == previoustext) {
						click thepresetloc[yloop-1]
						TheMultiplier := RegExReplace(RegExReplace(RegExReplace(OCR.FromRect(ocrdetectionthingx, ocrdetectionthingy, ocrdetectionthingw, ocrdetectionthingh, "en-us", 2).Text, "i)[OQ]", "0"), "i)[Il|]", "1"), "[^0-9.]", "")
						ttp previoustext "`n" TheMultiplier, 400, 400 ,1
					}else{
						ttp previoustext "`n" TheMultiplier, 400, 400 ,1
						previoustext := TheMultiplier
						break
					}
					sleep 50
					if (tries >= 20) {
					...
					try {
						if (TheMultiplier >= kgThreshold) {
						goalreach := 1
						break
					}
					}catch as e{
						; msgbox "error message: " e.message "`n`nVariable: " TheMultiplier
					}
				}
				try {
					if (TheMultiplier >= kgThreshold) {
...
;				sleep delay
			...
			msgbox "Got what you want`nKG: " TheMultiplier "`nThreshold you set: " kgThreshold, "Goal Achieved", "0x40040"
			goalreach := 0
			cleartooltip
			CurrentlyRunning := 0
			highlight()
		...
		if (CurrentlyRunning == 0) {
			msgbox "Force Stopped", "Stopped", "0x40030"
			cleartooltip
			goalreach := 0
			highlight()
			break
		...
		if (MoneyUsage >= moneyThreshold) {
			msgbox "Money Threshold reached`nThreshold: " moneyThreshold "`nMoney you spend: " MoneyUsage, "You broke ahh bih", "0x40030"
			cleartooltip
			goalreach := 0
			CurrentlyRunning := 0
			highlight()
...
AppraiseThing() {
global
	if !(xdiff or ydiff or mousex or mousey) {
		msgbox "buh config it first before you complain and yap in the questions channel", "error", "0x40030"
	return
	}
	StopwatchStart := A_TickCount
	CurrentlyRunning := 1
	...
	loop {
		...
		if !(goalreach) {
			loop {
				...
				click mousex+(xdiff*random(0,2)), mousey+(ydiff*yloop)
				yloop++
				sleep delay
				TheMultiplier := RegExReplace(RegExReplace(RegExReplace(OCR.FromRect(ocrdetectionthingx, ocrdetectionthingy, ocrdetectionthingw, ocrdetectionthingh, "en-us", 2).Text, "i)[OQ]", "0"), "i)[Il|]", "1"), "[^0-9.]", "")
				tries := 0
				loop {
					if (TheMultiplier == previoustext) {
						click mousex+(xdiff*random(0,2)), mousey+(ydiff*(yloop-1))
						TheMultiplier := RegExReplace(RegExReplace(RegExReplace(OCR.FromRect(ocrdetectionthingx, ocrdetectionthingy, ocrdetectionthingw, ocrdetectionthingh, "en-us", 2).Text, "i)[OQ]", "0"), "i)[Il|]", "1"), "[^0-9.]", "")
						ttp previoustext "`n" TheMultiplier, 400, 400 ,1
					}else{
						ttp previoustext "`n" TheMultiplier, 400, 400 ,1
...
TimerTest() {
global
	Thething := A_TickCount-StopwatchStart
	s := floor(Thething/1000)
	if (s >= 60) {
	m := s // 60
	s := floor(mod(s, 60))
	}
	...
	thefulldata.value := "Mouse Position: " mousex ", " mousey "`n-Difference-`nx: " xdiff "`ny: " ydiff "`nEstimated Runtime: " format("{:02}:{:02}:{:02}", h,m,s) "`nC$ Spent: " MoneyUsage " C$"
}
...
global
	SetXYDistance(0)
	SetOCRBoxThing(0)
	reappraiseboxsettingthing(0)
}

SetOCRBoxThing(saveit := 1) {
global
	resizeablebar("Set the box to the KG message`n(Leftmost doesnt touch the `"e`" in Appraise)`n(rightmost is set to the edge of kg)", "r", "0xFFE100", 150, 50)
	ocrdetectionthingx:=Xx
	ocrdetectionthingy:=Yy
	ocrdetectionthingw:=Width
	ocrdetectionthingh:=Height
	msgbox "OCR RANGE:`nx: " ocrdetectionthingx "`ny: " ocrdetectionthingy "`nw: " ocrdetectionthingw "`nh: " ocrdetectionthingh, "Confirmation", "0x40040"
	if (saveit) {
	Updatesettings
	}
}

reappraiseboxsettingthing(saveit := 1) {
global
	resizeablebar("Set the box to the Reappraise", "r",, 160, 60)
	reappraisex := left
	reappraisey := top
	reappraisex2 := right
	reappraisey2 := bottom
	msgbox "REAPPRAISE RANGE:`nx: " reappraisex "`ny: " reappraisey "`nx2: " reappraisex2 "`ny2: " reappraisey2, "Confirmation", "0x40040"
	if (saveit) {
	Updatesettings
	}
}

SetXYDistance(saveit := 1) {
global
	tol := 5
	cleartooltip
	resizeablebar("Set the box to the middle of the top left questionmark`n`n(dont put in the edges)`n(resizeable)", "r")
	if (pixelsearch(&importantTLx, &importantTLy, left, top, right, bottom, "0xA5A6A5", tol)) {
		ttp ".",importantTLx,importantTLy,19
	}
	resizeablebar("Set your mouse to the middle of the second row middle questionmark`n`n(dont put in the edges)`n(resizeable)", "r")
	if (pixelsearch(&importantSMx, &importantSMy, left, top, right, bottom, "0xA5A6A5", tol)) {
		ttp ".",importantSMx,importantSMy,18
	}
	resultty := msgbox("if you see both dots press `"Yes`", else press no and restart config", "Confirmation", "0x40044")
	if (resultty == "No") {
		cleartooltip
		return
	}
	sleep 100
	try {
		xdiff := importantSMx-importantTLx
		ydiff := importantSMy-importantTLy
	}catch as e {
		msgbox "ERROR: " e.message "`n`ngo retake the settings again", "error", "0x40030"
		return
	}
	tooltiprn := 1
	xlimit := 0
	ylimit := 0
	loop {
		ttp tooltiprn, importantTLx+(xdiff*xlimit), importantTLy+(ydiff*ylimit),tooltiprn
		if (tooltiprn >= 20) {
			break
		}
		xlimit++
	...
	msgbox("Do you see all 20 tooltip align with the questionmark?`n(ignore bottom right)`n`nif you dont see it just restart config", "confirmation", "0x40040")
	cleartooltip()
	mousex := importantTLx
	mousey := importantTLy
	if (saveit) {
...
resizeablebar(messages := "", keystowait := "r", color := "0x00ff00", TemporaryWidth := "100", TemporaryHeight := "70") {
global
	ScaleGUI := Gui("+ToolWindow +Resize -Caption +AlwaysOnTop -Border")
	ScaleGUI.backcolor := color
	ScaleGUI.Show()
	MouseGetPos(&xxxxx1, &yyyyy1)
	whereitwouldbe := xxxxx1-TemporaryWidth//2
	whereitactuallyis := yyyyy1-TemporaryHeight//2
	...
	FunctionICopyFromAutoAppraisal2 := (*) => ttp(messages "`n`nPress `"" keystowait "`" to continue",,,20)
	settimer FunctionICopyFromAutoAppraisal2, 100
	keywait keystowait, "D"
	keywait keystowait
	settimer FunctionICopyFromAutoAppraisal2, 0
	ttp ,,, 20
	...
	for i, v in strsplit(yeah, "`n") {
		if (trim(v) != "") {
			try {
				...
				if (arr1 != "thepresetloc") {
					%arr1% := arr2
				}else{
					thepresetloc := []
					tempor := strsplit(arr2, "-")
					for i, v in tempor
					{
						if (trim(v) != "") {
							thepresetloc.push(trim(v))
...
global
	...
	thegui.setfont("s13 cwhite norm w800", "Tahoma")
	themadebylolzzn := thegui.add("text", "x60 y35", "made by lolzzn")
	makeitallalign(themadebylolzzn)
	thegui.setfont("s15 cwhite underline w800", "Tahoma")
	thetreasuretitle := thegui.add("text", "x13 y5", "Positive Debt Gain")
	makeitallalign(thetreasuretitle)
	thegui.add("groupbox", "x10 y50 w230 h" guiheight-60)
	thegui.setfont("s12 cwhite norm w800", "Tahoma")
	delaytext := thegui.add("text", "x20 y70", "Delay")
	makeitallalign(delaytext)
	Thresholdtext := thegui.add("text", "x20 y125", "Weight Threshold")
	makeitallalign(Thresholdtext)
	Thresholdmoney := thegui.add("text", "x20 y180", "Money Threshold")
	makeitallalign(Thresholdmoney)
	thegui.add("groupbox", "x15 y240 w220 h120", "Data")
	
	retakeeverything := thegui.add("button", "x15 y370", "Retake Everything")
	retakeeverything.onevent("click", (*) => WriteDefaultSettings())
	makeitallalign(retakeeverything)
	retakedifference := thegui.add("button", "x15 y410", "Retake Difference")
	retakedifference.onevent("click", (*) => SetXYDistance())
	makeitallalign(retakedifference)	
	retakeocrsetting := thegui.add("button", "x15 y450", "Retake OCR Location")
	retakeocrsetting.onevent("click", (*) => SetOCRBoxThing())
	makeitallalign(retakeocrsetting)
	retakereappraiseloc := thegui.add("button", "x15 y490", "Retake Reappraise Loc")
	retakereappraiseloc.onevent("click", (*) => reappraiseboxsettingthing())
	makeitallalign(retakereappraiseloc)
	takepresetloc := thegui.add("button", "x15 y530", "Take Preset Location")
	takepresetloc.onevent("click", (*) => TakePRESSETLOCKATION())
	makeitallalign(takepresetloc)
	
	thegui.setfont("s10 cwhite norm w400", "Tahoma")
	thedelay := thegui.add("edit", "x20 y95 +background3b3b3b w210", delay)
	thedelay.onevent("change", (*) => Updatesettings())
	SendMessage(0x1501, false, StrPtr("Delay"), thedelay.Hwnd)
	
	theweightThreshold := thegui.add("edit", "x20 y150 +background3b3b3b w210", kgThreshold)
	theweightThreshold.onevent("change", (*) => Updatesettings())
	SendMessage(0x1501, false, StrPtr("Weight Threshold"), theweightThreshold.Hwnd)
	
	themoneythreshold := thegui.add("edit", "x20 y205 +background3b3b3b w210", moneyThreshold)
	themoneythreshold.onevent("change", (*) => Updatesettings())
	SendMessage(0x1501, false, StrPtr("Weight Threshold"), theweightThreshold.Hwnd)
	
	thefulldata := thegui.add("text", "x20 y260 w200", "Mouse Position: " mousex ", " mousey "`n-Difference-`nx: " xdiff "`ny: " ydiff "`nEstimated Runtime: " format("{:02}:{:02}:{:02}", h,m,s) "`nC$ Spent: " MoneyUsage " C$")
	thegui.title := "Treasure Appraisal V1"
	thegui.show("w" guiwidth " h" guiheight)
	thegui.onevent("close", (*) => exitapp())
	thegui.setfont("s10 c0xbfbfbf norm w400", "Tahoma")
	thegui.add("text", "x20 y570", "f1 to exit | f2 to reload`nctrl + q to start normal`nctrl + r to start preset`nalt to force stop")
	thetreasuretitle.focus()
}

makeitallalign(GUINAME) {
global
	GUINAME.getpos( ,, &tempyheay)
	GUINAME.move(floor(guiwidth/2-tempyheay/2))
}
...
global
	...
	"xdiff=" xdiff "`n"
	"ydiff=" ydiff "`n"
	"ocrdetectionthingx=" ocrdetectionthingx "`n"
	"ocrdetectionthingy=" ocrdetectionthingy "`n"
	"ocrdetectionthingw=" ocrdetectionthingw "`n"
	"ocrdetectionthingh=" ocrdetectionthingh "`n"
	"reappraisex=" reappraisex "`n"
	"reappraisey=" reappraisey "`n"
	"reappraisex2=" reappraisex2 "`n"
	"reappraisey2=" reappraisey2 "`n"
	"delay=375`n"
	"kgThreshold=1.5`n"
	"moneyThreshold=9999999999`n"
	"mousex=" mousex "`n"
	"mousey=" mousey "`n"
	"thepresetloc="  "`n"
	), File1
...
global
	...
	"reappraisex2=" reappraisex2 "`n"
	"reappraisey2=" reappraisey2 "`n"
	"delay=" trim(thedelay.value) "`n"
	"kgThreshold=" trim(theweightThreshold.value) "`n"
	"moneyThreshold=" trim(themoneythreshold.value) "`n"
	"mousex=" mousex "`n"
	"mousey=" mousey "`n"
	"thepresetloc=" tempora "`n"
	), File1
	readthemfile()
	thefulldata.value := "Mouse Position: " mousex ", " mousey "`n-Difference-`nx: " xdiff "`ny: " ydiff "`nEstimated Runtime: " format("{:02}:{:02}:{:02}", h,m,s) "`nC$ Spent: " MoneyUsage " C$"
}
...
FileAppend
	...
	"xdiff=0
	ydiff=0
	ocrdetectionthingx=0
	ocrdetectionthingy=0
	ocrdetectionthingw=0
	ocrdetectionthingh=0
	reappraisex=0
	reappraisey=0
	reappraisex2=0
	reappraisey2=0
